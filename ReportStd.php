...
<html lang="en">
  ...
  <body>
    <div class="shadow p-3 mb-5 bg-body rounded container">
        ...
        //   echo '';
        // }
          $sql = "SELECT rollNumber FROM addstudent WHERE usernameStd = '$username'";
          $sqlres = mysqli_query($conn, $sql);
          while($row = mysqli_fetch_assoc($sqlres)){
            $id = $row['rollNumber'];
          }
          $sql = "SELECT * FROM issuebook where RollNo = '$id'";
          ...
          if($num1==0){
            echo'<p class="text-danger"> &nbsp;&nbsp;No book issued for '.$id.' till date.</p>';
          }
          $b =1;
          $a=1;
          while($row = mysqli_fetch_array($result)){
            $roll = $row['RollNo'];
            $sql = "SELECT `firstName`, `lastName` FROM `addStudent` WHERE rollNumber = '$roll'";
            $stdn = mysqli_query($conn, $sql);
            while($column = mysqli_fetch_array($stdn)){
              $fullname = $column['firstName'] . $column['lastName'];
            }
            // $sql = "SELECT `Fine` FROM `issueBook` WHERE ROllNo = $roll";
            // $stdfine = mysqli_query($conn, $sql);
            // while($column = mysqli_fetch_array($stdfine)){
            //   $fine = $column['Fine'];
            // }
            // if($fine ==""){
            //   $fine = '-';
            // }
              ...
              <div class="my-1" style="clear: left;">
                ...
                <!-- <div class="mx-1" style ="width:auto; text-align:center; display:inline-block;"><b>Fine</b></div> -->
                <div class="mx-1" style ="width:25%; text-align:center; display:inline-block;"><b>Book Title</b> </div>
                <div class="mx-1" style ="width:25%; text-align:center; display:inline-block;"><b> Branch</b></div> 
                </div><hr><br>
              <?php $b--; } 
              if($row['ReturnDate']==''){
                ?>
              ...
              <div class="my-1">
                ...
                <!-- <div class="mx-1" style ="width:12%; text-align:center; display:inline-block;"> <?php echo $fine ?> </div> -->
                <div class="mx-1" style ="width:25%; text-align:center; display:inline-block;">&nbsp; <?php echo $row['Title'] ?> </div>
                <div class="mx-1" style ="width:25%; text-align:center; display:inline-block;"> <?php echo $row['Branch'] ?> </div> 
                </div><hr>
